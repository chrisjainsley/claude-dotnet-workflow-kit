#!/usr/bin/env python
"""Fail loudly when a review.md breaks the section skeleton or its word budget.

Usage: python check_review.py plans/<slug>/review.md [--profile <dotnet-workflow-kit.json>]

The section list is the same for every architecture; --profile is accepted so the
command line matches check_plan.py and so future profile-driven rules have a home.
...
from profile import add_profile_arg, resolve_profile  # noqa: E402
...
REQUIRED_META = ("title", "ticket", "pr", "branch", "base")
...
def main(path):
    ...
    failures = [f"front matter is missing '{k}'" for k in REQUIRED_META if not meta.get(k)]

    found = split_sections(body)
    names = [n for n, _ in found]
    expected = [n for n, _ in SECTIONS]
    if names != expected:
    ...
    prose_only = re.sub(r"`[^`]*`", "", strip_fences(body))
    ...
    print(f"{'total prose':<28}{total:>7}{round(sum(c for _, c in SECTIONS) * multiplier):>6}")
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    resolve_profile(explicit=args.profile)
